...
# ไม่เจาะจงเลขรอบ = เทียบกับทุกรอบที่ทำหลังจากนี้ — ถามว่า "ทำใหม่ให้ถูกแล้วยัง" ไม่ใช่ "รอบไหนทำ"
buf_all = io.StringIO()
with redirect_stdout(buf_all):
    audit.main(["--run", "1"])
alls = buf_all.getvalue()
check("ไม่ใส่เลขรอบ ก็เทียบกับทุกรอบที่ทำทีหลัง", "เทียบกับรอบที่ทำหลังจากนี้ (#2" in alls)
check("และยังได้คำตอบเดิม", "ซ้ำกับรอบ #1: 1 เที่ยว" in alls)
