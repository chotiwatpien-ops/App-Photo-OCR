...
| เมนู | มีอะไร |
|---|---|
| **หน้าหลัก** | สถานะการดูดรูปจาก Drive รอบล่าสุด + ปุ่ม "▶ ดูดรูปตอนนี้" (สั่ง GitHub Actions; ต้องตั้ง `github_token` + `github_repo` ใน config) · สรุปราย**สัปดาห์ → กลุ่มรถ → ไรเดอร์** (รูป/งาน/อนุมัติ/รอคน/รายได้) · ลิงก์ตรงไป Excel และโฟลเดอร์รูปส่งลูกค้าบน Drive · อัปโหลดมือซ่อนอยู่ด้านล่าง (กรณีนอกรอบ) |
| **คิวตรวจ (N)** | แถวที่ระบบไม่กล้าอนุมัติเอง **จัดกลุ่มตามไรเดอร์** พร้อมเหตุผล (ซ้ำใน job / เคยอนุมัติแล้ว job #X / เลขขัดกัน / ข้อมูลไม่พอ / ไม่มีวันที่) · ดูรูป · อนุมัติ / ลบ ทีละแถว · "อนุมัติที่เหลือทั้งหมด" ต่อไรเดอร์ · เปิด job เพื่อแก้ตัวเลข |
